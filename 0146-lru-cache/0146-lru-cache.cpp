// A  LRU cache is Ds that stores limited number of items and evicts LRU item when capacity is exceeded. It supports get and put operations and our goal is to keep recently accessed items esaily available and remove older ones. 
// implement LRU cache class that implements contructor that takes constructor where it takes capacity and initialize capacity. 

// Questions:- 
// 1. Can we assume that the capacity is always a positive integer greater than zero?
// 2. Are the key and value both integers, or should the cache support more generic types?
// 3.Should we overwrite the value if the same key is inserted again using put(key, value)? 

// so , to achieve O(1) time for both get and put i can thinnk of is using hashmap - to store key and a pointer to node in DLL.(Node position) 
// DLL - every node will  store actual key value pair  
// Dummy head previous points to null and next to tail   and dummy tail previous points to head and nexct to null.
// and whenevr we will access (get) a key - move that node top front (most recent) and when (put) a key - if key exist , update its value and move front and if does not exist we will add a new node at front and if cache exceeds the capacity  we will remove the node at tail (least reecntly used). 
// Time complexity :-  O(1)
// space complexity :- O(N) - hasmap and DLL both takes O(N)
...
public:
    ...
    unordered_map<int, Node*> m; // Hash map: key → node pointer
    ...
    // Helper function to add a node right after the head (mark as most recently used)
    ...
    void put(int key, int value) {
        ...
        if (m.find(key) != m.end()) {
            ...
            if (m.size() == cap) {
                Node* lru = tail->prev;  // Get the least recently used node (just before tail)
